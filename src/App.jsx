      ...
      <div style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1 }}>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '40px 20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24, animation: 'fade-in 0.6s ease 0.1s both' }}>
            ...
            <div onClick={() => startQuiz('exam', null)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '18px 22px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s ease', boxShadow: 'var(--shadow-sm)', color: 'var(--text)' }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 700, letterSpacing: '-0.005em', color: 'var(--text)' }}>模擬試験</div>
                <div style={{ fontSize: '12.5px', color: 'var(--text-dim)', lineHeight: 1.5 }}>4時間 制限時間付き</div>
              </div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', padding: '5px 10px', background: 'var(--surface-3)', border: '1px solid var(--border)', borderRadius: 999, color: 'var(--text-dim)' }}>本番</div>
            </div>
          </div>
  ...
  if (stage === 'quiz' && questions.length > 0) {
    ...
    return (
      <div style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', zIndex: 1 }}>
        <Ambient />
        <Grid />
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 20px', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, marginBottom: 24, animation: 'slide-up 0.4s ease' }}>
            <div style={{ padding: 32, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, boxShadow: 'var(--shadow-md)' }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: 'rgba(255, 140, 66, 0.12)', border: '1px solid rgba(255, 140, 66, 0.4)', borderRadius: 8, fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#ff8c42', marginBottom: 16, fontFamily: "'JetBrains Mono', monospace" }}>
                {DOMAINS.find(d => d.id === q.domain)?.name}
              </div>
              <div style={{ fontSize: 18, fontWeight: 600, lineHeight: 1.7, color: 'var(--text)', marginBottom: 28, letterSpacing: '-0.002em' }}>
                {q.question}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24 }}>
                {q.options.map((opt, idx) => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: '14.5px', lineHeight: 1.75, color: 'var(--text)', flex: 1 }}>
                      {opt}
                    </div>
                  </div>
                ))}
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'space-between' }}>
            <button onClick={prevQuestion} disabled={currentQuestion === 0} style={{ flex: 1, padding: '14px 20px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, cursor: currentQuestion === 0 ? 'not-allowed' : 'pointer', fontWeight: 600, color: '#ff8c42', fontSize: 14, transition: 'all 0.15s ease', opacity: currentQuestion === 0 ? 0.5 : 1 }}>← 前へ</button>
            <button onClick={nextQuestion} style={{ flex: 1, padding: '14px 20px', background: 'linear-gradient(180deg, #ff8c42 0%, #e67e30 100%)', border: 'none', borderRadius: 12, cursor: 'pointer', fontWeight: 600, color: '#ffffff', fontSize: 14, transition: 'all 0.15s ease', boxShadow: '0 8px 20px rgba(255, 140, 66, 0.3)' }}>次へ →</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', zIndex: 1 }}>
        ...
        <div style={{ maxWidth: 600, margin: '0 auto', padding: '40px 20px' }}>
          <div style={{ fontSize: 32, fontWeight: 700, textAlign: 'center', marginBottom: 40, animation: 'slide-up 0.5s ease' }}>試験終了</div>

          <div style={{ position: 'relative', width: 220, height: 220, margin: '0 auto 40px', animation: 'fade-in 0.6s ease 0.2s both' }}>
            <div style={{
              width: '100%',
              height: '100%',
              borderRadius: '50%',
              ...
              <div style={{ width: 180, height: 180, borderRadius: '50%', background: 'var(--surface)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                <div style={{ fontSize: 48, fontWeight: 800, color: '#ff8c42' }}>{percentage}%</div>
                <div style={{ fontSize: 12, color: 'var(--text-dim)', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.1em', textTransform: 'uppercase' }}>正解率</div>
              </div>
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, width: '100%', marginBottom: 32, animation: 'fade-in 0.6s ease 0.3s both' }}>
            <div style={{ padding: 20, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, textAlign: 'center', boxShadow: 'var(--shadow-sm)' }}>
              <div style={{ fontSize: 28, fontWeight: 700, color: '#ff8c42', marginBottom: 4 }}>{score}</div>
              <div style={{ fontSize: 12, color: 'var(--text-dim)', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.08em', textTransform: 'uppercase' }}>正解数</div>
            </div>
            <div style={{ padding: 20, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, textAlign: 'center', boxShadow: 'var(--shadow-sm)' }}>
              <div style={{ fontSize: 28, fontWeight: 700, color: '#ff8c42', marginBottom: 4 }}>{questions.length}</div>
              <div style={{ fontSize: 12, color: 'var(--text-dim)', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.08em', textTransform: 'uppercase' }}>総問数</div>
            </div>
          </div>

          <div style={{ width: '100%', marginBottom: 32, animation: 'fade-in 0.6s ease 0.4s both' }}>
            {domainResults.map(result => (
              <div key={result.domain} style={{ padding: 16, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, boxShadow: 'var(--shadow-sm)' }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>{result.domain}</div>
                <div style={{ fontSize: 14, fontWeight: 700, color: '#ff8c42', fontFamily: "'JetBrains Mono', monospace" }}>{result.score}/{result.total}</div>
              </div>
            ))}
          </div>

          <div style={{ display: 'flex', gap: 12, width: '100%', animation: 'fade-in 0.6s ease 0.5s both' }}>
            <button onClick={() => location.reload()} style={{ flex: 1, padding: '14px 20px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, cursor: 'pointer', fontWeight: 600, color: 'var(--text)', fontSize: 14, transition: 'all 0.15s ease' }}>もう一度</button>
            <button onClick={goToTitle} style={{ flex: 1, padding: '14px 20px', background: 'linear-gradient(180deg, #ff8c42 0%, #e67e30 100%)', border: 'none', borderRadius: 12, cursor: 'pointer', fontWeight: 600, color: '#ffffff', fontSize: 14, transition: 'all 0.15s ease', boxShadow: '0 8px 20px rgba(255, 140, 66, 0.3)' }}>メニューへ</button>
          </div>
        </div>
      </div>
    );
